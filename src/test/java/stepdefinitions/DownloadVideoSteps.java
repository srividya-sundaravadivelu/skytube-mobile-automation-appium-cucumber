...
public class DownloadVideoSteps {
	...
	public void the_downloads_tab_should_display_a_message_download_some_videos_to_find_them_here() {
	    ...
	    WebElement emptyMessageElement = wait.until(ExpectedConditions
	        .visibilityOf(downloadVedioPage.downloadSomeVediosMsg));
		...
		downloadVedioPage.clickThreeDotsOfDownloads();
		 ...
		 WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(5));
		    WebElement deleteOption = wait.until(ExpectedConditions
		        .visibilityOf(downloadVedioPage.deleteDownloadOption));  // Adjust if needed
		    deleteOption.click();
	...
	public void the_selected_video_should_be_deleted_from_downloads_tab() {
		//downloadVedioPage.isDownloadMsgDisplayed();
		 WebDriverWait wait = new WebDriverWait(getDriver(), Duration.ofSeconds(10));
		    ...
		    boolean isDeleted;
		    try {
		        isDeleted = wait.until(ExpectedConditions
		            .invisibilityOf(downloadVedioPage.titleOfDownloadedVedio));
		    } catch (Exception e) {
		        isDeleted = false;
		    }
